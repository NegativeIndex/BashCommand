...
pols=["Ex","Ey","Ez"]
geoms=["Empty","PEC",'AL','AL0']
h_src1=np.arange(0.02,0.14,0.02)
h_src2=np.arange(0.15,1.00,0.05)
hs_src=np.concatenate((h_src1,h_src2))
...
for i,pol in enumerate(pols):
    for j,geom in enumerate(geoms):
        if geom=='Empty':
            ...
            for k,h_src in enumerate(hs_src):
                sig='{}_{}_H{:0.3f}'.format(geom,pol,h_src)
                os.chdir(path)
                simu_fun(i,j,k,sig)
                count+=1
